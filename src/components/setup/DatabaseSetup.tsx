...
CREATE POLICY IF NOT EXISTS "Users can delete own accounts" ON accounts
...
CREATE POLICY IF NOT EXISTS "Users can update own transactions" ON transactions
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own transactions" ON transactions
  FOR DELETE USING (auth.uid() = user_id);

-- Create RLS policies for loans
CREATE POLICY IF NOT EXISTS "Users can view own loans" ON loans
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own loans" ON loans
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own loans" ON loans
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own loans" ON loans
  FOR DELETE USING (auth.uid() = user_id);
...
-- Success message
SELECT 'Database setup complete! Your finance app is ready to use.' as result;`
          ...
          <div className="space-y-6">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <ul className="list-disc list-inside mt-2 text-green-700 space-y-1">
                <li>3 main tables (accounts, transactions, loans)</li>
                <li>Row Level Security for data protection</li>
                <li>Account balance update function</li>
